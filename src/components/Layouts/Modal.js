...
const ChangePasswordModal = ({ isOpen, onRequestClose , product }) => {
  ...
  return (
    ...
    <Modal
      ...
      className="absolute top-1/2 left-1/2 max-w-lg p-5 -translate-x-1/2 -translate-y-1/2 bg-navy rounded shadow-lg outline-none rounded-xl w-full sm:min-w-[40%]"
    ...
    >
      ...
      <div className="sm:w-full md:flex-1 min-w-0 bg-red-500 flex flex-col justify-start p-4">
        <p className="text-white text-lg font-bold md:text-2xl lg:text-3xl">{product.name}</p>
        <p className="text-pink text-xl md:text-xl lg:text-xl mb-8">{product.category.toUpperCase()}</p>
        <p className="text-white text-2xl font-semibold md:text-2xl lg:text-2xl">$ <span className='font-bold'>{product.price}</span></p>
      </div>
    </div>

    <div className="flex flex-col justify-center items-center sm:justify-start sm:items-start mt-5 gap-4">
      <h1 className='text-center sm:text-left text-xl md:text-2xl font-bold text-white mt-1'>Product Information:</h1>
     ...
     <div className="flex flex-wrap justify-between items-center mt-8 mb-3 gap-4">
      <button onClick={onRequestClose} className="bg-navy hover:bg-red text-white font-bold py-2 border border-pink px-4 rounded md:w-2/5 w-3/4 sm:w-2/3">
        Cancel
      </button>
